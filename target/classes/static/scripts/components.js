...
class Wine_Cards {
    ...
    filter_min_price(min_price, cards) {
        let matched_cards = [];
        for (let card of cards) {
            if (card.wine.purchasePrice >= min_price) {
                matched_cards.push(card);
            }
        }
        return matched_cards;
    ...
    filter_max_price(max_price, cards) {
        let matched_cards = [];
        for (let card of cards) {
            if (card.wine.purchasePrice <= max_price) {
                matched_cards.push(card);
            }
        }
        return matched_cards;
    ...
    init() {
        ...
        for (let wine of wines) {
            ...
            let params = {
                ...
                body: {
                    price: prettify_decimal(wine.purchasePrice, 2),
                    location: wine.location,
                    quantity: wine.quantity,
                },
            };
            this.wine_cards.create_card(wine.id, wine, params);
        }
